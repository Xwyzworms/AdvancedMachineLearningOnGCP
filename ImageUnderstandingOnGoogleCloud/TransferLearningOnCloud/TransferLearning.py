...
PROJECT : str = "qwiklabs-gcp-03-3416ced62aae"
BUCKET : str = "qwiklabs-gcp-03-3416ced62aae"
...
CONTRAST_UPPER = 1.8 # Upper boiund ( COntrastnya lebih contrast lagi karena diatas 1)
...
layers = [
	...
	Conv2D(num_filters_1, kernel_size_1, padding='same',
        activation='relu',input_shape=[IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS]),
	MaxPooling2D(pool_size=pooling_size_1),
	Conv2D(num_filters_2, kernel_size_2, padding='same',activation="relu"),
	MaxPooling2D(pool_size=pooling_size_2),
	Flatten(),
...
eval_ds = load_dataset(eval_path, BATCH_SIZE)

# %%
model_stracts = Sequential(layers)
model_stracts.compile(
	optimizer="adam",
	loss="categorical_crossentropy",
	metrics=["accuracy"]
)
model_stracts.fit_generator(train_ds, epochs=2, steps_per_epoch=5,validation_data=eval_ds,validation_steps=VALIDATION_STEPS)
# %%

#Hre using mobile net from tensorflow hub
model_names = "mobilenet_v2_100_224"
module_handle = "https://tfhub.dev/google/imagenet/{}/feature_vector/4".format(model_names)

transfer_model = tf.keras.Sequential([
	
	hub.KerasLayer(module_handle, trainable=False),
	Dropout(dropout_rate),
	Dense(nclasses, activation='softmax',kernel_regularizer=tf.keras.regularizers.l2(0.001))
])

# %%
transfer_model.build((IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))
...
transfer_model.fit(train_ds, epochs=2, steps_per_epoch=5,validation_data=eval_ds,validation_steps=VALIDATION_STEPS)
# %%
dasdas